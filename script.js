...
if(groundImg){
  // можно раскомментировать если нужно сдвинуть ещё
...
// применяем стили загрузки (фон чёрный, прогресс белый)
...
function fakeLoad(onDone){
  ...
  const interval = setInterval(() => {
    ...
    if(progress >= 100){
      ...
      // немного стилизуем чтобы не ломать общий вид
      playBtn.style.background = "#fff";
      playBtn.style.color = "#000";
      playBtn.style.border = "none";
      playBtn.style.borderRadius = "8px";
      playBtn.id = "splashPlayBtn";

      splashScreen.appendChild(playBtn);

      playBtn.onclick = () => {
        // пробуем проиграть каждый звук, чтобы "разблокировать" аудио-контекст
        try { sClickButton.volume = 0.0; playSound(sClickButton); sClickButton.volume = 1.0; } catch(e){}
        try { sClickClicker.volume = 0.0; playSound(sClickClicker); sClickClicker.volume = 1.0; } catch(e){}
        try { sClickWood.volume = 0.0; playSound(sClickWood); sClickWood.volume = 1.0; } catch(e){}

        // плавное затухание splash
        splashScreen.style.transition = "opacity 0.9s";
...
/* УБИРАЕМ ДУБЛИРУЮЩИЙ СЧЁТЧИК ПОД КЛИКЕРОМ */
const counterEl = document.getElementById('counter');
if (counterEl) {
  // скрываем элемент, чтобы оставался только счёт на табличке
...
function animateClicker(){
  ...
  if(clickImg) clickImg.src = "img/click2.png";
  setTimeout(()=>{
    if(clickImg) clickImg.style.transform = "scale(1)";
    if(clickImg) clickImg.src = "img/click1.png";
  }, 100);
}
